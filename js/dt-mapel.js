...
var table = $('#data-mapel').DataTable({
                  ...
                  $('.reset').trigger('click');
                   if(cek) {
                     $(".reset").on('click', function(){
                       //$("#jurusan, #id_mapel").selectpicker('deselectAll');
                     });
                   }
            }
         }
  ...
  $("#data-mapel").on("click", ".edit", function(e){
    e.stopPropagation();
  });
  $("#data-mapel").on("click", ".hapus", function(e){
    e.stopPropagation();
       ...
       var id = $(this).attr('id');
        swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               var data = $(this).serialize()+"&p=mapel&mod=hapus&id="+id;
               $.ajax({
                 url: "prosess.php",
                 data: data,
                 type: "POST",
   ...
   $("#ftambah").submit(function(e){
     e.preventDefault();
     var data = $(this).serialize()+"&p=mapel&mod=tambah";
     $.ajax({
       url : "prosess.php",
       data:data,
       type: "POST",
   ...
   $("#fedit").submit(function(e){
     e.preventDefault();
     var data = $(this).serialize()+"&p=mapel&mod=edit";
     $.ajax({
       url: "prosess.php",
       data: data,
       type: "POST",
