...
var table = $('#data-jurusan').DataTable({
                  ...
                  $('.reset').trigger('click');
                   if(cek) {
                     $(".reset").on('click', function(){
                       //$("#jurusan, #id_jurusan").selectpicker('deselectAll');
                     });
                   }
            }
         }
  ...
  $("#data-jurusan").on("click", ".edit", function(e){
    e.stopPropagation();
  });
  $("#data-jurusan").on("click", ".hapus", function(e){
    e.stopPropagation();
       ...
       var id = $(this).attr('id');
        swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               var data = $(this).serialize()+"&p=jurusan&mod=hapus&id="+id;
               $.ajax({
                 url: "prosess.php",
                 data: data,
                 type: "POST",
   ...
   $("#ftambah").submit(function(e){
     e.preventDefault();
     var data = $(this).serialize()+"&p=jurusan&mod=tambah";
     $.ajax({
       url : "prosess.php",
       data:data,
       type: "POST",
   ...
   $("#fedit").submit(function(e){
     e.preventDefault();
     var data = $(this).serialize()+"&p=jurusan&mod=edit";
     $.ajax({
       url: "prosess.php",
       data: data,
       type: "POST",
